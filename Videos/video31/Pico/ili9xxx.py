...
#             pix_format = [0x66]
        ...
        self._run_seq(
            [
                ...
                (_VMCTR1, b"\x3E\x28"),  # VCOM ctrl 1
                (_VMCTR2, b"\x86"),  # VCOM ctrl 2
                (_MADCTL, b"\x48"),  # Memory access ctrl
                (_VSCRSADD, b"\x00"),  # Vertical scrolling start address
                (_PIXFMT, b"\x55"),  # COLMOD: Pixel format
                (_FRMCTR1, b"\x00\x18"),  # Frame rate ctrl
                (_DFUNCTR, b"\x02,x02"),
...
#                 (_GMCTRN1, bytes([0x00, 0x16, 0x19, 0x03, 0x0F, 0x05, 0x32, 0x45, 0x46, 0x04, 0x0E, 0x0D, 0x35, 0x37, 0x0F])),  #e1                
                (_GMCTRP1, bytes([0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08, 0x4E, 0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00])),  #xe0
                (_GMCTRN1, bytes([0x00, 0x0E, 0x14, 0x03, 0x11, 0x07, 0x31, 0xC1, 0x48, 0x08, 0x0F, 0x0C, 0x31, 0x36, 0x0F])),  #e1
    ...
    def apply_rotation(self, rot):
        print("ili9488 apply_rotation")
        self.rot = rot
        if (self.rot % 2) == 0:
            print("rot%2,res:",self.rot,self.res)
            self.width, self.height = self.res
        else:
            self.height, self.width = self.res
#         print("self.width,height:",self.width, self.height)
#         print("self.rot,madctl_rot:",self.rot,_MADCTL_ROTS[self.rot%4])
#         print("madctl_bgr, BGR|ROTS:",_MADCTL_BGR,_MADCTL_BGR | _MADCTL_ROTS[self.rot % 4])
        self.write_register(
            _MADCTL,
            bytes([_MADCTL_BGR | _MADCTL_ROTS[self.rot % 4]]),
        )
...
class Ili9488(Ili9488_hw, st77xx.St77xx_lvgl):
    def __init__(self, res, doublebuffer=True, factor=8, **kw):
        """See :obj:`Ili9488_hw` for the meaning of the parameters."""
        print("Ili9488 driver")
        Ili9488_hw.__init__(self, res, **kw)
        st77xx.St77xx_lvgl.__init__(self, res, doublebuffer, factor)
...
class St7796_hw(st77xx.St77xx_hw):
    ...
    def apply_rotation(self, rot):
        self.rot = rot
        _MADCTL_BGR = 0x04  #change to RGB
        if (self.rot % 2) == 0:
            self.width, self.height = self.res
        else:
            self.height, self.width = self.res
...
#             self.write_register(
#                 ST77XX_MADCTL,
#                 bytes([(ST77XX_MADCTL_BGR if self.bgr else 0)|ST77XX_MADCTL_ROTS[self.rot%4]]))
