...
_INTMODE = const(0xB0)  # Interface Mode Control
...
_ENTMODE = const(0xB7) # Entry Mode Set
...
class Gc9a01_hw(st77xx.St77xx_hw):
    ...
    def apply_rotation(self, rot):
        ...
        if (self.rot % 2) == 0:
            ...
            self.width, self.height = self.res
        try:
            self.write_register(const(0x3A),b"\x55")
#             self.write_register(
#                 _MADCTL,
#                 bytes([_MADCTL_BGR | _MADCTL_ROTS[self.rot % 4]]),
#             )
...
class Ili9488_hw(st77xx.St77xx_hw):
    ...
    def config_hw(self):
        self._run_seq(
            [
                ...
                (
                    _GMCTRP1,
                    b"\x00\x03\x09\x08\x16\x0A\x3F\x78\x4C\x09\x0A\x08\x16\x1A\x0F",
                ),
                (
                    _GMCTRN1,
                    b"\x00\x16\x19\x03\x0F\x05\x32\x45\x46\x04\x0E\x0D\x35\x37\x0F",
                ),
                #(_PWCTRB, b"\x00\xC1\x30"),  # Pwr ctrl B
                #(_POSC, b"\x64\x03\x12\x81"),  # Pwr on seq. ctrl
                #(_DTCA, b"\x85\x00\x78"),  # Driver timing ctrl A
                ...
                #(_PWCTR2, b"\xC1\x41"),  # Pwr ctrl 2  works
                (_VMCTR1, b"\x00\x12\x80"),  # VCOM ctrl 1
                #(_VMCTR2, b"\x86"),  # VCOM ctrl 2
                (_VSCRSADD, b"\x00"),  # Vertical scrolling start address
                (_PIXFMT, b"\x55"),  # COLMOD: Pixel format
                (_INTMODE, b"\x00"),
                (_FRMCTR1, b"\xA0"),  # Frame rate ctrl
                (_INVCTR, b"\x02"),   # Display Inversion
                (_DFUNCTR, b"\x02\x02\x3b"),
                (_ENTMODE, b"\xC6"),
                (_PUMPRC, b"\xA9\x51\x2C\x82"),  # Pump ratio control
    ...
    def apply_rotation(self, rot):
        ...
        if (self.rot % 2) == 0:
            ...
            #self.width, self.height = self.res
        #self.height, self.width = self.res
        print("ili9488 RES:",self.res)
        try:
            #print("0x3A Pixel interface format:%x" % [_MADCTL_BGR | _MADCTL_ROTS[self.rot % 4]])
            #print("0x3A format:%x" % _MADCTL)
            self.write_register(
                _MADCTL,
                bytes([_MADCTL_BGR | _MADCTL_ROTS[self.rot % 4]]),
            )
            #self.write_register( const(0x36),b"\x48")
        except:
            print("except in ili9xxx, _madctl:",_MADCTL)
...
class Ili9488b_hw(st77xx.St77xx_hw):
    def __init__(self, **kw):
        """ILI9488b TFT Display Driver."""
        self.display_type = "ili9488b"
        print("ILI9488b_hw")
        super().__init__(
            res=(320, 480),
            suppRes=[
    ...
    def config_hw(self):
        self._run_seq(
            [
                ...
                (_VMCTR1, b"\x00\x12\x80"),  # VCOM ctrl 1
                ( const(0x36), b"\x48" ),
                (_PIXFMT, b"\x66"),  # COLMOD: Pixel format
                (_INTMODE, b"\x00"),  # Interface mode control
                (_FRMCTR1, b"\xA0"),  # Frame rate ctrl
                (_INVCTR, b"\x02"),   # Display Inversion
                (_DFUNCTR, b"\x02\x02\x3b"),   # Display Function Control
                (_ENTMODE, b"\xC6"),  # Entry Mode Set
                (_PUMPRC, b"\xa9\x51\x2c\x82"),  # Pump ratio control/ Adjust cntl 3
                (_SLPOUT, None, 120),
                (_DISPLAY_ON, None, 25),
            ]
        )
...
class Ili9488b(Ili9488b_hw, st77xx.St77xx_lvgl):
    def __init__(self, doublebuffer=True, factor=8, **kw):
        """See :obj:`Ili9488b_hw` for the meaning of the parameters."""
        ######_MADCTL_BGR must = 0x04  see apply _rotation()
        Ili9488b_hw.__init__(self, **kw)
        st77xx.St77xx_lvgl.__init__(self, doublebuffer, factor)
