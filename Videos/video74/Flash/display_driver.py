...
import machine
from machine import SPI, Pin, reset, SoftSPI
import time
import os
import sys
...
#disp = st77xx.St7735( spi=spi, dc=8, cs=9, rst=12, bl=13, rot=0, res=(128,160), model="greentab", bgr=False)
# Xiia 2.4
#disp = ili9xxx.Ili9341(spi=spi, dc=8, cs=9, rst=12, rot=LANDSCAPE)
# 1.3
#disp = st77xx.St7789( spi=spi, dc=8, cs=9, rst=12, bl=13, rot=PORTRAIT, res=(240,320), model=None)
disp = st77xx.St7789( spi=spi, dc=8, cs=9, rst=12, bl=13, rot=PORTRAIT, res=(172,320), model="172")

#disp = ili9xxx.Gc9a01(spi=spi, dc=0, cs=17, rst=1, rot=PORTRAIT)
#disp = ili9xxx.Ili9341(spi=spi, dc=0, cs=17, rst=1, rot=LANDSCAPE)
#disp.set_model("big")  # uncomment for ILI9341 2.8 and 3.2 inch display
#disp = ili9xxx.St7796(spi=spi, dc=0, cs=17, rst=1, rot=PORTRAIT)
...
hres = disp.width   
vres = disp.height
print("disp.width(hres),disp.height(vres):",hres,vres)

disp_drv = lv.display_create(hres,vres)
scr = lv.screen_active()

touch = True
if disp.display_type in ['gca901','st7789','st7735']:
      touch = False
