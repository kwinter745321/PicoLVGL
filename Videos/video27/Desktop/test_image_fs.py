...
def showImage(filename):
    ...
    try:
        #
        # read and structure not needed
        #
        img1 = lv.image(scr)
        img1.set_src(filename)
...
def my_open_cb(drv, filename, mode):
    with open(filename, mode) as f:
        drv.file_d = f
        return 0
    return 1

def my_read_cb( fs_file, buf, btr, br):
    try:
...
fs_driver.open_cb = my_open_cb
fs_driver.read_cb = my_read_cb
fs_driver.close_cb = my_close_cb

fs_driver.fs_register(fs_drv,'S')

#### Main ###############################################
filename = 'S:dir1/cog1.png'
